...
select * from e_commerce_orders;
select * from e_commerce_orders order by order_id;
select * from e_commerce_orders group by prodect_name='laptop';
...
select * from e_commerce_orders limit 5 offset 17;
select * from e_commerce_orders where customer_name like 'R%';
select * from e_commerce_orders where price between 100 and 500;
select * from e_commerce_orders where not  product_name='Smartphone';
select * from e_commerce_orders where quantity>1;
select count(prodect_name) from e_commerce_orders  group by customer_name='Ravi Kumar';
select count(prodect_name) from e_commerce_orders where prodect_name='Laptop';
select * from e_commerce_orders;

select distinct(customer_name) from e_commerce_orders where order_date='2025-10-10';
select * from e_commerce_orders where prodect_name='Laptop';
...
select * from e_commerce_orders where order_id % 2=0;
SELECT *FROM (SELECT *,ROW_NUMBER() OVER (ORDER BY order_id) AS rn FROM e_commerce_orders) AS t WHERE (rn - 1) % 3 = 0;
select * from (select *,row_number() over (order by order_id) as rn from e_commerce_orders) as t where (rn - 1) %4 = 0;
select * from (select*,row_number() over (order by order_id) as jg from e_commerce_orders) as t where (jg-1) %7 = 0;
select * from e_commerce_orders where order_id % 2=0;
